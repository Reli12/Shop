...
userRouter.get(
  ...
  expressAsyncHandler(async (req, res) => {
    //this is for removing all users in table
    // await User.remove({});
...
userRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    //password is saved in hash becouse of protection
    //saved user in database
    const createdUser = await user.save();
    //send to frontend for new user
    res.send({
      _id: createdUser._id,
...
userRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (user) {
      //if it is empty string then use name in database
